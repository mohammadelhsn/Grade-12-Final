...
class Ball:
    img:str;speed:int
    def __init__(self,img:str)->None:self.img=img;self.ball=Image(Point(random.randint(-350,350),250),self.img);self.speed=0
    ...
    def moveDown(self,screen:GraphWin):
        if((self.ball.anchor.y-(self.speed))<=-250):game.score+=1;game.score_button.text.setText(f"Score:{game.score}");self.ball.undraw();self.ball.anchor.x=random.randint(-350,350);self.ball.anchor.y=250;self.draw(screen)
        else:
...
class Game:
    ...
    def defaultScreen(self):
        ...
        if (self.score != 0): 
            Button(Point(-100, -80), Point(100, -40), f"LAST SCORE: {self.score}", self.default_screen).activate().setFontSize(16).draw()
        while True:
            ...
            if(SETTINGS_BUTTON.clicked(pt)):SETTINGS_BUTTON.clickAnimation();defaultScreen.close(); res = self.settings();return res;break
            if(PLAY_BUTTON.clicked(pt)):PLAY_BUTTON.clickAnimation();defaultScreen.close();res = self.gameScreen();return res
        defaultScreen.close()
    def gameScreen(self):
        ...
        while True:
            ...
            if(len(self.balls) != 7 and self._settings.difficulty=="medium"):speed+=1;self.addBall(Ball(f"ball1.gif").setSpeed(speed))
            ...
            for ball in self.balls:
                if(self.players[0].hasCollided(ball.ball.anchor.x,ball.ball.anchor.y,ball.ball.getHeight(),ball.ball.getWidth(),self.players[0].current.anchor.x,self.players[0].current.anchor.y,self.players[0].current.getHeight(),self.players[0].current.getWidth())):
                    if(ball.ball.id in recentlyCollided):continue
                        ...
                        if (ball.isDrawn(gameScreen) == "no"): pass
                        self.players[0].lives-=1;self.lives_button.text.setText(f'Lives {self.players[0].lives}');recentlyCollided.append(ball.ball.id)
    ...
    def settings(self):
        ...
        while True:
            ...
            if(AFTERPARTY.clicked(pt)):AFTERPARTY.clickAnimation();AFTERPARTY.activate();BIGGESTBIRD.deactivate();RICKROLL.deactivate();self._settings.music.playSong("afterparty.wav")
            if(BIGGESTBIRD.clicked(pt)):BIGGESTBIRD.clickAnimation();BIGGESTBIRD.activate();AFTERPARTY.deactivate();RICKROLL.deactivate();self._settings.music.playSong("biggestbird.wav")
            if(RICKROLL.clicked(pt)):RICKROLL.clickAnimation();RICKROLL.activate();AFTERPARTY.deactivate();BIGGESTBIRD.deactivate();self._settings.music.playSong("rickroll.wav")
            ...
            if BACK.clicked(pt):BACK.clickAnimation();settingsScreen.close();return self._settings;break
            if(EASY.clicked(pt)):EASY.clickAnimation();self._settings.updateDifficulty("easy");EASY.activate();MEDIUM.deactivate();HARD.deactivate()
            if(MEDIUM.clicked(pt)):MEDIUM.clickAnimation();self._settings.updateDifficulty("medium");MEDIUM.activate();EASY.deactivate();HARD.deactivate()
            if(HARD.clicked(pt)):HARD.clickAnimation();self._settings.updateDifficulty("hard");HARD.activate();EASY.deactivate();MEDIUM.deactivate()
            continue
